...
export const SidebarNavigation: React.FC<SidebarNavigationProps> = ({
  ...
  const menuItems = [
    ...
    { id: 'hutang_piutang' as ActiveTab, label: 'HUTANG & PIUTANG', sublabel: 'Pantau Cicilan & Pinjaman', icon: Scale, badge: debtCount > 0 ? `${debtCount} ACTIVE` : null, badgeColor: 'bg-amber-500/20 text-amber-300 border-amber-500/30' },
    ...
    <aside className="w-full h-full bg-[#0c0717] p-4 lg:p-5 flex flex-col justify-between shadow-[4px_0_20px_rgba(168,85,247,0.08)]">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full text-left px-3 py-2.5 rounded-xl transition-all flex items-center justify-between group cursor-pointer ${isActive ? 'bg-gradient-to-r from-purple-600/35 via-fuchsia-600/25 to-purple-800/30 text-white border-2 border-fuchsia-400 shadow-[0_0_20px_rgba(217,70,239,0.35)]' : 'text-purple-200/70 hover:text-white hover:bg-purple-500/15 border border-purple-500/20 hover:border-purple-400/40'}`}>
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${isActive ? 'bg-gradient-to-tr from-purple-500 via-fuchsia-500 to-pink-500 text-white shadow-[0_0_12px_rgba(217,70,239,0.6)]' : 'bg-[#160d29] text-fuchsia-400 border border-purple-500/30'}`}><Icon className="w-4 h-4" /></div>
      ...
      <div className="mt-6 pt-4 border-t border-purple-500/20 space-y-3">
        <div className="bg-[#140b24] border border-purple-500/35 rounded-xl p-3 space-y-2 relative overflow-hidden shadow-[inset_0_0_15px_rgba(168,85,247,0.15)]">
          <div className="flex items-center gap-2 text-purple-200"><Sparkles className="w-4 h-4 text-fuchsia-400 animate-pulse" /><h4 className="text-xs font-orbitron font-bold text-white uppercase tracking-tighter">Rinjani AI Assistant</h4></div>
          <p className="text-[10px] text-purple-200/80 leading-relaxed font-rajdhani font-medium">Analisis kas harian, rekomendasi hemat, dan prediksi portofolio otomatis!</p>
          <button onClick={onOpenAIModal} className="w-full bg-gradient-to-r from-purple-600 via-fuchsia-500 to-purple-700 hover:from-purple-500 hover:to-fuchsia-400 text-white font-orbitron font-bold text-xs py-2 rounded-lg flex items-center justify-center gap-1.5 shadow-neo-purple border border-fuchsia-300/30 cursor-pointer transition-all active:scale-95"><Sparkles className="w-3.5 h-3.5 fill-white text-white" /><span>KONSULTASI AI</span></button>
        </div>

        <button onClick={() => { if (window.confirm("Keluar dari sistem?")) onLogout?.(); }} className="w-full bg-[#180a0a] hover:bg-rose-950/60 border border-rose-500/40 hover:border-rose-400 text-rose-400 font-orbitron font-bold text-xs py-2.5 rounded-xl flex items-center justify-center gap-2 shadow-neo-rose cursor-pointer transition-all active:scale-95"><LogOut className="w-4 h-4" /><span>LOGOUT SYSTEM</span></button>
      </div>
    </aside>
  );
};
