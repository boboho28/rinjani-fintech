    ...
    <aside className="w-full lg:w-80 bg-[#0c0717] border-b lg:border-b-0 lg:border-r border-purple-500/25 p-4 lg:p-5 shrink-0 flex flex-col justify-between shadow-[4px_0_20px_rgba(168,85,247,0.08)] lg:sticky lg:top-[112px] lg:h-[calc(100vh-112px)] overflow-y-auto">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-1.5 pt-1">
          <div onClick={onOpenRateModal} className="bg-[#140b24] border border-purple-500/30 rounded-lg p-1.5 text-center flex flex-col items-center justify-center hover:border-purple-400/60 transition-all cursor-pointer group">
          ...
          <div onClick={onOpenCryptoModal} className="bg-[#140b24] border border-purple-500/30 rounded-lg p-1.5 text-center flex flex-col items-center justify-center hover:border-purple-400/60 transition-all cursor-pointer group">
          ...
          <div onClick={onOpenGoldModal} className="bg-[#140b24] border border-purple-500/30 rounded-lg p-1.5 text-center flex flex-col items-center justify-center hover:border-purple-400/60 transition-all cursor-pointer group">
        ...
        <div className="px-1 pt-2">
          <p className="text-[10px] font-orbitron font-bold uppercase tracking-widest text-purple-300/70">MAIN ACCESS MENU</p>
        </div>
        ...
        <nav className="grid grid-cols-1 gap-2">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${isActive ? 'bg-gradient-to-tr from-purple-500 via-fuchsia-500 to-pink-500 text-white shadow-[0_0_12px_rgba(217,70,239,0.6)]' : 'bg-[#160d29] text-fuchsia-400 border border-purple-500/30'}`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  <div>
                    <p className={`text-xs font-orbitron font-bold tracking-wider ${isActive ? 'text-fuchsia-300' : 'text-slate-200'}`}>{item.label}</p>
                    <p className="text-[10px] text-purple-300/60 font-rajdhani font-semibold truncate max-w-[140px]">{item.sublabel}</p>
                  </div>
                </div>
      ...
      <div className="mt-6 pt-4 border-t border-purple-500/20 space-y-3">
        <div className="bg-[#140b24] border border-purple-500/35 rounded-xl p-3 space-y-2 relative overflow-hidden shadow-[inset_0_0_15px_rgba(168,85,247,0.15)]">
          ...
          <button onClick={onOpenAIModal} className="w-full bg-gradient-to-r from-purple-600 via-fuchsia-500 to-purple-700 text-white font-orbitron font-bold text-xs py-2 rounded-lg flex items-center justify-center gap-1.5 shadow-neo-purple border border-fuchsia-300/30 cursor-pointer">
        ...
        <button
          onClick={() => { if (window.confirm("Apakah Anda yakin ingin keluar?")) onLogout?.(); }}
          className="w-full bg-[#180a0a] hover:bg-rose-950/60 border border-rose-500/40 hover:border-rose-400 text-rose-400 font-orbitron font-bold text-xs py-2.5 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer"
        >
