...
const getGeminiClient = () => {
  ...
  if (!apiKey) {
    console.warn('GEMINI_API_KEY environment variable is not set. AI features will run in offline mode.');
    return null;
  }
...
app.post('/api/ai/parse-transaction', async (req, res) => {
  try {
    ...
    if (!ai) {
      // Fallback parser if API key is not present
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`KeuanganKu AI Server active at http://0.0.0.0:${PORT}`);
  });
}
